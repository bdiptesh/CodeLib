...
class Tree():
    ...
    def _compute_metrics(self):
        ...
        if self.method in ("classify", "regression"):
            ...
            y_hat = list(self.model.predict(self.ts_df[self.ts_x_var]))
    ...
    def _ts_data_transform(self):
        ...
        df = pd.DataFrame({"lag": list(range(self.max_lag+1)),
                           "pacf": pacf(df, nlags=self.max_lag, method='ols')})
        df["thres_val"] = (np.round(norm.ppf(1 - (self.threshold / 2)), 2)
                           / (len(self.df) ** 0.5))
        ...
        df = df.dropna()
        self.ts_lag_var = df['lag'].astype(int).to_list()
        self.ts_lag_var.append(m)
        for i in self.ts_lag_var:
            if i == 0:
    ...
    def _ts_predict(self,
        ...
        for i in range(0, len(df_op)):
            ...
            y_hat = self.model.predict(df_pred_x[self.ts_x_var])
    ...
    def _fit(self) -> Dict[str, Any]:
        ...
        if self.param is None:
            ...
            if self.method == "classify":
                ...
                self.param["max_features"] \
                    = [int(np.ceil(len(self.ts_x_var) / 3))]
                self.param["min_samples_leaf"] = [5]
                self.param["max_depth"] = list(range(1, len(self.ts_x_var)))
        if self.method == "classify":
    ...
    def _fit(self) -> Dict[str, Any]:
        ...
        if self.param is None:
            self.param = {"n_estimators": [100],
                          ...
                          "min_child_weight": [0.5, 1.0, 3.0]}
            if self.method == "timeseries":
                self.param["max_depth"] = [int(len(self.ts_x_var) * 0.8)]
            elif self.method in ("regression", "classify"):
